...
class BackupGenerator:
	...
	def take_dump(self):
		...
		for t in tables:
			cmd = f"""nohup mysqldump --single-transaction --quick --lock-tables=false --compress=true --add-locks --skip-comments -u"{args['user']}" -p"{args['password']}" -h"{args['db_host']}" -P"{args['db_port']}" "{args['db_name']}" "{t[0]}" | gzip > "{path}/{t[0]}.sql.gz" &"""
			err, out = frappe.utils.execute_in_shell(cmd)
